...
- Full encryption implemented for the transport layer.
  ...
  - Hence, communication is encrypted and transported. Upon reaching the destination, the packet will be decrypted. This means that both the server and client can view content sent locally. It is not E2EE.
...
- plug and play - setting up a server is required,
...
- GUI forks of polycomm are *NOT* officially endorsed. Use them with caution.
...
- Messages stored locally, in `.[server-ip].json` files.
  - You cannot see messages that were sent while you were not connected[^6].

## Setup:
- Clone the repository using `git`.
  - `git clone https://github.com/Polymorqhism/polycomm.git`
- Enter the polycomm directory using `cd polycomm`.
- Ensure your version of Linux is inclusive of [socket](https://pubs.opengroup.org/onlinepubs/7908799/xns/syssocket.h.html), [ncurses](https://pubs.opengroup.org/onlinepubs/7908799/xcurses/curses.h.html) and [libsodium](https://libsodium.gitbook.io/doc) libraries.
- Compile the project:
  - Simply typing `make` should compile the project.
- Finally, to run the binary located in the `build/` directory.
- Complete setup (after dependency checks):
```
git clone https://github.com/Polymorqhism/polycomm.git
cd polycomm
make
```
- To run the project, you can run `make run`, or run it directly from the binary. Optionally, to access it globally (RECOMMENDED):
```
sudo mv build/polycomm /usr/bin/
```
...
[^3]: polycomm uses threads per client, on the server. This is the simplest setup that can be implemented while having the least complexity and best performance. Each client is handled independently.
...
[^6]: The server does not sync messages for simplicity and security reasons. polycomm only stores messages during the connection.
